    ...
    return (
        <div className="flex flex-col lg:h-full gap-4 ">
            <div className="grid  flex-1   grid-cols-[1fr] lg:grid-cols-[1fr_1fr]  gap-4  min-h-0  ">
                <div
                    className="border  firstrow  bg-[#E8E5FF]  flex justify-between  items-center sm:w-full p-2 sm:p-4 rounded-2xl shrink">
                ...
                <Allmodelsstats prediction={prediction}/>
            </div>


            <div
                className="lg:flex-[1.2] border grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 p-4  h-[200px]  overflow-y-scroll bg-[#232B43] rounded-xl">


                <div className="min-w-0 flex flex-col gap-2">
                    <h1 className="self-center text-[#8B5CF6]">XGBoost Regressor</h1>
                    <MetricBars
                        data={data["XGBoost"] || []}
                        color="#8B5CF6"
                    />
                </div>
                <div className="min-w-0 flex flex-col gap-2 text-[#14B8A6]" >
                    <h1 className="self-center">Random Forest</h1>
                    <MetricBars data={data["RandomForest"] || []} color="#14B8A6"/>
                </div>
                <div className="min-w-0 flex flex-col gap-2 text-[#F59E0B]">
                    <h1 className="self-center">Linear Regression</h1>
                    <MetricBars data={data["Linear"] || []} color="#F59E0B"/>
                </div>

                <div className="min-w-0 flex flex-col gap-2 text-[#EF6A5B]">
                    <h1 className="self-center">Lasso Regression</h1>
                    <MetricBars data={data["Lasso"] || []} color="#EF6A5B"/>
                </div>

            </div>


            <div className="grid  gris-cols-[1fr] lg:grid-cols-[1fr_1fr] md:flex-1 gap-4  rounded-xl  ">
                <div className="border min-h-[30vh] bg-[#232B43] rounded-xl">
                    <RsqaureChart/>
                </div>
                <div className="border  min-h-[30vh] bg-[#232B43]  rounded-xl p-4">
                    <KmVsPriceChart/>
                </div>
            </div>
